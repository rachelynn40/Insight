...
from becquerel.core import fitting
from becquerel.core.utils import handle_uncs, handle_datetime, bin_centers_from_edges
from becquerel.parsers import SpeFile, CnfFile
...
class Spectrum(object):
    ...
    def has_uniform_bins(self, use_kev=None, rtol=None):
        ...
        if rtol is None:
            rtol = 100 * becquerel.utils.EPS
        if rtol < becquerel.utils.EPS:
    ...
    def plot(self, *fmt, **kwargs):
        ...
        plotter = SpectrumPlotter(self, *fmt, **kwargs)
        fig, ax = plotter.plot()
    ...
    def fill_between(self, **kwargs):
        ...
        plotter = SpectrumPlotter(self, **kwargs)
...
def _get_file_object(infilename):
    ...
    if extension.lower() == '.spe':
        return SpeFile(infilename)
    elif extension.lower() == '.spc':
        return becquerel.SpcFile(infilename)
    elif extension.lower() == '.cnf':
        return CnfFile(infilename)
    else:
        raise NotImplementedError(
